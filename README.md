...
Installable Loop Engineering Pack for Harness-based agent work.

This repo adds a reusable loop layer on top of
[`repository-harness`](https://github.com/hoangnb24/repository-harness):

- `docs/loops/core-loop.md` for every task.
- Domain loops for web build, web app, API/backend, data pipeline, automation,
  research/docs, and security-risk work.
- A small verifier that checks the loop pack is installed and wired into
  `AGENTS.md`.

## Install Into A Project

First install the base Harness if the target project does not have it yet:
...
Then install the Loop Engineering Pack:
...
If you publish this under another repository, set the source explicitly:

```bash
LOOP_ENGINEERING_REPO="your-org/your-repo" \
curl -fsSL "https://raw.githubusercontent.com/your-org/your-repo/main/scripts/install-loop-engineering.sh?$(date +%s)" | bash -s -- --merge --yes
```

```powershell
$env:LOOP_ENGINEERING_REPO = "your-org/your-repo"
& ([scriptblock]::Create((irm "https://raw.githubusercontent.com/your-org/your-repo/main/scripts/install-loop-engineering.ps1"))) -Merge -Yes
```
...
Create or open the GitHub repo `hailongluu/loops-engineering`, then from this folder:

```bash
git init
git add .
git commit -m "Add installable loop engineering pack"
git branch -M main
git remote add origin https://github.com/hailongluu/loops-engineering.git
git push -u origin main
```

After that, the install commands above will work from raw GitHub URLs.
